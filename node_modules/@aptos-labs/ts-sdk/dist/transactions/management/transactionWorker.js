 ...
 * @category Transactions
 */
export var TransactionWorkerEventsEnum;
(function (TransactionWorkerEventsEnum) {
    // fired after a transaction gets sent to the chain
    TransactionWorkerEventsEnum["TransactionSent"] = "transactionSent";
    // fired if there is an error sending the transaction to the chain
    TransactionWorkerEventsEnum["TransactionSendFailed"] = "transactionSendFailed";
    // fired when a single transaction has executed successfully
    TransactionWorkerEventsEnum["TransactionExecuted"] = "transactionExecuted";
    // fired if a single transaction fails in execution
    TransactionWorkerEventsEnum["TransactionExecutionFailed"] = "transactionExecutionFailed";
    // fired when the worker has finished its job / when the queue has been emptied
    TransactionWorkerEventsEnum["ExecutionFinish"] = "executionFinish";
})(TransactionWorkerEventsEnum || (TransactionWorkerEventsEnum = {}));
/**
...
export class TransactionWorker extends EventEmitter {
     ...
     * @category Transactions
     */
    outstandingTransactions = new AsyncQueue();
    /**
     ...
     * @private
     */
    addToTransactionHistory(history, entry) {
        history.push(entry);
     ...
     * @category Transactions
     */
    constructor(aptosConfig, account, maxWaitTime = 30, maximumInFlight = 100, sleepTime = 10) {
        super();
        this.aptosConfig = aptosConfig;
        this.account = account;
        this.started = false;
        this.accountSequnceNumber = new AccountSequenceNumber(aptosConfig, account, maxWaitTime, maximumInFlight, sleepTime);
    }
    /**
     ...
     * @category Transactions
     */
    async push(transactionData, options) {
