...
import { AnyPublicKey, AnySignature, Secp256k1PublicKey, MultiKey, MultiKeySignature, } from "../../core/crypto/index.js";
...
import { AccountAuthenticator, AccountAuthenticatorEd25519, AccountAuthenticatorMultiEd25519, AccountAuthenticatorMultiKey, AccountAuthenticatorNoAccountAuthenticator, AccountAuthenticatorSingleKey, } from "../authenticator/account.js";
import { TransactionAuthenticatorEd25519, TransactionAuthenticatorFeePayer, TransactionAuthenticatorMultiAgent, TransactionAuthenticatorMultiEd25519, TransactionAuthenticatorSingleSender, } from "../authenticator/transaction.js";
import { ChainId, EntryFunction, FeePayerRawTransaction, MultiAgentRawTransaction, MultiSig, MultiSigTransactionPayload, RawTransaction, Script, TransactionPayloadEntryFunction, TransactionPayloadMultiSig, TransactionPayloadScript, } from "../instances/index.js";
...
import { TransactionExecutableEmpty, TransactionExecutableEntryFunction, TransactionExecutableScript, TransactionExtraConfigV1, TransactionInnerPayloadV1, } from "../instances/transactionPayload.js";
 ...
 */
export async function generateTransactionPayload(args) {
    if (isScriptDataInput(args)) {
        ...
        if ("multisigAddress" in args) {
            ...
            return new TransactionPayloadMultiSig(new MultiSig(multisigAddress, new MultiSigTransactionPayload(scriptPayload.script)));
        }
 ...
 * @category Transactions
 */
export function generateTransactionPayloadWithABI(args) {
    ...
    const functionArguments = args.functionArguments?.map((arg, i) => 
    /**
    ...
    if ((functionArguments?.length ?? 0) !== functionAbi.parameters.length) {
        throw new Error(`Too few arguments for '${moduleAddress}::${moduleName}::${functionName}', expected ${functionAbi.parameters.length} but got ${functionArguments?.length ?? 0}`);
    }
 ...
 * @category Transactions
 */
export async function generateViewFunctionPayload(args) {
    const { moduleAddress, moduleName, functionName } = getFunctionParts(args.function);
    const functionAbi = await fetchAbi({
        key: "view-function",
        moduleAddress,
 ...
 * @category Transactions
 */
export function generateViewFunctionPayloadWithABI(args) {
    const functionAbi = args.abi;
    const { moduleAddress, moduleName, functionName } = getFunctionParts(args.function);
    // Ensure that all type arguments are typed properly
    const typeArguments = standardizeTypeTags(args.typeArguments);
...
function generateTransactionPayloadScript(args) {
    return new TransactionPayloadScript(new Script(Hex.fromHexInput(args.bytecode).toUint8Array(), standardizeTypeTags(args.typeArguments), args.functionArguments));
}
/**
...
export async function generateRawTransaction(args) {
    ...
    if (options?.encrypted && gasUnitPrice < BigInt(MIN_ENCRYPTED_TXN_GAS_UNIT_PRICE)) {
        if (options.gasUnitPrice !== undefined) {
            ...
            console.warn(`Encrypted transaction: gasUnitPrice ${gasUnitPrice} is below the minimum ${MIN_ENCRYPTED_TXN_GAS_UNIT_PRICE}; overriding to ${MIN_ENCRYPTED_TXN_GAS_UNIT_PRICE}.`);
        }
        gasUnitPrice = BigInt(MIN_ENCRYPTED_TXN_GAS_UNIT_PRICE);
    }
    const expireTimestamp = options?.expireTimestamp ?? BigInt(Math.floor(Date.now() / 1000) + aptosConfig.getDefaultTxnExpirySecFromNow());
    const replayProtectionNonce = options?.replayProtectionNonce !== undefined ? BigInt(options.replayProtectionNonce) : undefined;
    ...
    return new RawTransaction(AccountAddress.from(sender), BigInt(sequenceNumber), txnPayload, BigInt(maxGasAmount), BigInt(gasUnitPrice), BigInt(expireTimestamp), new ChainId(chainId));
}
export function convertPayloadToInnerPayload(payload, replayProtectionNonce) {
    if (payload instanceof TransactionPayloadScript) {
        return new TransactionInnerPayloadV1(new TransactionExecutableScript(payload.script), new TransactionExtraConfigV1(undefined, replayProtectionNonce));
    }
    if (payload instanceof TransactionPayloadEntryFunction) {
        return new TransactionInnerPayloadV1(new TransactionExecutableEntryFunction(payload.entryFunction), new TransactionExtraConfigV1(undefined, replayProtectionNonce));
    }
    if (payload instanceof TransactionPayloadMultiSig) {
        ...
        return new TransactionInnerPayloadV1(executable, new TransactionExtraConfigV1(payload.multiSig.multisig_address, replayProtectionNonce));
    }
...
export async function buildTransaction(args) {
    ...
    if ("secondarySignerAddresses" in args) {
        const signers = args.secondarySignerAddresses?.map((signer) => AccountAddress.from(signer)) ?? [];
        return new MultiAgentTransaction(rawTxn, signers, args.feePayerAddress ? AccountAddress.from(args.feePayerAddress) : undefined);
    }
 ...
 * @category Transactions
 */
export async function generateSignedTransactionForSimulation(args) {
    const { signerPublicKey, transaction, secondarySignersPublicKeys, feePayerPublicKey } = args;
    const accountAuthenticator = await getAuthenticatorForSimulation(signerPublicKey);
    // fee payer transaction
    if (transaction.feePayerAddress) {
        const transactionToSign = new FeePayerRawTransaction(transaction.rawTransaction, transaction.secondarySignerAddresses ?? [], transaction.feePayerAddress);
        let secondaryAccountAuthenticators = [];
        if (transaction.secondarySignerAddresses) {
            if (secondarySignersPublicKeys) {
                secondaryAccountAuthenticators = await Promise.all(secondarySignersPublicKeys.map((publicKey) => getAuthenticatorForSimulation(publicKey)));
            }
            else {
                secondaryAccountAuthenticators = await Promise.all(Array.from({ length: transaction.secondarySignerAddresses.length }, () => getAuthenticatorForSimulation(undefined)));
            }
        }
        const feePayerAuthenticator = await getAuthenticatorForSimulation(feePayerPublicKey);
        const transactionAuthenticator = new TransactionAuthenticatorFeePayer(accountAuthenticator, transaction.secondarySignerAddresses ?? [], secondaryAccountAuthenticators, {
    ...
    if (transaction.secondarySignerAddresses) {
        const transactionToSign = new MultiAgentRawTransaction(transaction.rawTransaction, transaction.secondarySignerAddresses);
        let secondaryAccountAuthenticators = [];
        if (secondarySignersPublicKeys) {
            secondaryAccountAuthenticators = await Promise.all(secondarySignersPublicKeys.map((publicKey) => getAuthenticatorForSimulation(publicKey)));
        }
        ...
        const transactionAuthenticator = new TransactionAuthenticatorMultiAgent(accountAuthenticator, transaction.secondarySignerAddresses, secondaryAccountAuthenticators);
        return new SignedTransaction(transactionToSign.raw_txn, transactionAuthenticator).bcsToBytes();
    }
 ...
 * @category Transactions
 */
export async function getAuthenticatorForSimulation(publicKey) {
    if (!publicKey) {
    ...
    const detectKeylessVariant = (key) => {
        if ("jwkAddress" in key && "keylessPublicKey" in key)
            return AnyPublicKeyVariant.FederatedKeyless;
        if ("iss" in key && typeof key.iss === "string" && "idCommitment" in key)
    ...
    if (AnyPublicKey.isInstance(accountPublicKey)) {
        if (accountPublicKey.variant === AnyPublicKeyVariant.Keyless ||
            ...
            return new AccountAuthenticatorSingleKey(accountPublicKey, new AnySignature(KeylessSignature.getSimulationSignature()));
        }
    ...
    if (MultiKey.isInstance(accountPublicKey)) {
        let keylessSignatureModule;
        return new AccountAuthenticatorMultiKey(accountPublicKey, new MultiKeySignature({
...
export function generateSignedTransaction(args) {
    ...
    if (transaction.feePayerAddress) {
        ...
        txnAuthenticator = new TransactionAuthenticatorFeePayer(senderAuthenticator, transaction.secondarySignerAddresses ?? [], additionalSignersAuthenticators ?? [], {
            address: transaction.feePayerAddress,
            authenticator: feePayerAuthenticator,
        });
    }
        ...
        txnAuthenticator = new TransactionAuthenticatorMultiAgent(senderAuthenticator, transaction.secondarySignerAddresses, additionalSignersAuthenticators);
    }
    else if (senderAuthenticator instanceof AccountAuthenticatorEd25519) {
        txnAuthenticator = new TransactionAuthenticatorEd25519(senderAuthenticator.public_key, senderAuthenticator.signature);
    }
    else if (senderAuthenticator instanceof AccountAuthenticatorMultiEd25519) {
        txnAuthenticator = new TransactionAuthenticatorMultiEd25519(senderAuthenticator.public_key, senderAuthenticator.signature);
    }
    else {
 ...
 * @category Transactions
 */
export function hashValues(input) {
 ...
 * @category Transactions
 */
export function generateUserTransactionHash(args) {
...
async function fetchAbi({ key, moduleAddress, moduleName, functionName, aptosConfig, abi, fetch, }) {
    ...
    return memoizeAsync(async () => fetch(moduleAddress, moduleName, functionName, aptosConfig), `${key}-${aptosConfig.network}-${moduleAddress}-${moduleName}-${functionName}`, 1000 * 60 * 5)();
}
