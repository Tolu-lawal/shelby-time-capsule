...
import { TypeTagBool, TypeTagU8, TypeTagU16, TypeTagU32, TypeTagU64, TypeTagU128, TypeTagU256, TypeTagI8, TypeTagI16, TypeTagI32, TypeTagI64, TypeTagI128, TypeTagI256, TypeTagAddress, TypeTagStruct, TypeTagVector, TypeTagGeneric, StructTag, } from "../typeTag/index.js";
import { parseTypeTag } from "../typeTag/parser.js";
import { getModule } from "../../internal/account.js";
import { Bool, U8, U16, U32, U64, U128, U256, I8, I16, I32, I64, I128, I256, } from "../../bcs/serializable/movePrimitives.js";
 ...
 */
export class MoveStructArgument extends Serializable {
    /**
 ...
 */
export class MoveEnumArgument extends Serializable {
    /**
...
export class StructEnumArgumentParser {
     ...
     */
    async fetchModule(moduleAddress, moduleName) {
     ...
     */
    async encodeStructArgument(structTag, value, depth = 0) {
        ...
        if (!module.abi) {
            throw new Error(`Module ${structTag.value.address}${MODULE_SEPARATOR}${structTag.value.moduleName.identifier} has no ABI`);
        }
        // Find the struct definition
        const structDef = module.abi.structs.find((s) => s.name === structTag.value.name.identifier);
        if (!structDef) {
            throw new Error(`Struct ${structTag.value.name.identifier} not found in module ${structTag.value.address}${MODULE_SEPARATOR}${structTag.value.moduleName.identifier}`);
        }
        if (structDef.is_enum) {
            throw new Error(`Type ${structTag.value.name.identifier} is an enum. Use enum variant syntax instead (e.g., {"VariantName": {...}})`);
        }
     ...
     */
    async encodeEnumArgument(structTag, value, depth = 0) {
        ...
        if (!enumDef) {
            throw new Error(`Enum ${structTag.value.name.identifier} not found in module ${structTag.value.address}${MODULE_SEPARATOR}${structTag.value.moduleName.identifier}`);
        }
        if (!enumDef.is_enum) {
            throw new Error(`Type ${structTag.value.name.identifier} is a struct, not an enum. Provide field values directly as an object.`);
        }
        ...
        if (variantIndex === -1) {
            ...
            throw new Error(`Variant '${variantName}' not found in enum ${structTag.value.name.identifier}. Available variants: ${availableVariants}`);
        }
        const serializer = new Serializer();
            ...
            if (sortedKeys.length > 1) {
                for (let i = 0; i < sortedKeys.length; i++) {
                    ...
                    if (sortedKeys[i] !== expectedKey) {
                        throw new Error(`Enum variant with multiple fields must use sequential numeric keys starting from "0". ` +
     ...
     */
    async encodeOptionArgument(structTag, variant, fieldValue, depth) {
     ...
     */
    isOptionType(structTag) {
        return (structTag.value.address.toString() === "0x1" &&
     ...
     */
    substituteTypeParams(fieldType, structTag) {
        ...
        if (fieldType instanceof TypeTagGeneric) {
            ...
            if (index >= structTag.value.typeArgs.length) {
                throw new Error(`Generic type parameter T${index} out of bounds. ${structTag.value.name.identifier} has ${structTag.value.typeArgs.length} type arguments.`);
            }
        ...
        if (fieldType instanceof TypeTagStruct) {
            ...
            const substitutedTypeArgs = fieldType.value.typeArgs.map((typeArg) => this.substituteTypeParams(typeArg, structTag));
            // Create new struct tag with substituted type arguments
            const newStructTag = new StructTag(fieldType.value.address, fieldType.value.moduleName, fieldType.value.name, substitutedTypeArgs);
     ...
     */
    async encodeValueByType(typeTag, value, depth) {
     ...
     */
    async encodeVector(vectorType, value, depth) {
     ...
     */
    async encodeStruct(structTag, value, depth) {
     ...
     */
    parseNumberBigInt(value, typeName) {
...
//# sourceMappingURL=structEnumParser.js.map