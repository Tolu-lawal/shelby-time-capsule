...
import { MultiKey, MultiKeySignature } from "../core/crypto/index.js";
...
export class MultiKeyAccount {
     ...
     * @category Account (On-Chain Model)
     */
    signingScheme = SigningScheme.MultiKey;
    /**
    ...
    constructor(args) {
        ...
        const signers = args.signers.map((signer) => signer instanceof Ed25519Account ? SingleKeyAccount.fromEd25519Account(signer) : signer);
        if (multiKey.signaturesRequired > signers.length) {
            throw new Error(`Not enough signers provided to satisfy the required signatures. Need ${multiKey.signaturesRequired} signers, but only ${signers.length} provided`);
        }
        else if (multiKey.signaturesRequired < signers.length) {
            throw new Error(`More signers provided than required. Need ${multiKey.signaturesRequired} signers, but ${signers.length} provided`);
        }
