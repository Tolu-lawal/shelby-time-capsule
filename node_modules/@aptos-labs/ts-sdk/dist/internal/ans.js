...
import { ExpirationStatus, SubdomainExpirationPolicy, } from "../types/index.js";
 ...
 */
export function isValidANSSegment(fragment) {
    if (!fragment)
        return false;
    if (fragment.length < 3)
        return false;
    if (fragment.length > 63)
        return false;
 ...
 * @group Implementation
 */
export function isValidANSName(name) {
 ...
 * @group Implementation
 */
export function getANSExpirationStatus({ name, gracePeriod, }) {
    if (!name)
...
export const LOCAL_ANS_ACCOUNT_PK = getEnvVar("ANS_TEST_ACCOUNT_PRIVATE_KEY") ??
    "ed25519-priv-0x37368b46ce665362562c6d1d4ec01a08c8644c488690df5a17e13ba163e20221";
export const LOCAL_ANS_ACCOUNT_ADDRESS = getEnvVar("ANS_TEST_ACCOUNT_ADDRESS") ?? "0x585fc9f0f0c54183b039ffc770ca282ebd87307916c215a3e692f2f8e4305e82";
const NetworkToAnsContract = {
 ...
 * @group Implementation
 */
function getRouterAddress(aptosConfig) {
    const address = NetworkToAnsContract[aptosConfig.network];
    if (!address)
        throw new Error(`The ANS contract is not deployed to ${aptosConfig.network}`);
    return address;
}
const unwrapOption = (option) => {
...
export async function registerName(args) {
    ...
    const routerAddress = getRouterAddress(aptosConfig);
    const { domainName, subdomainName } = isValidANSName(name);
    const hasSubdomainPolicy = expiration.policy === "subdomain:independent" || expiration.policy === "subdomain:follow-domain";
    ...
    const expirationDateInMillisecondsSinceEpoch = expiration.policy === "subdomain:independent" ? expiration.expirationDate : tldExpiration;
...
export async function getPrimaryName(args) {
    ...
    const subdomainName = unwrapOption(res[0]);
    if (!domainName)
        return undefined;
 ...
 * @group Implementation
 */
export async function getAccountNames(args) {
 ...
 * @group Implementation
 */
export async function getAccountDomains(args) {
    const { aptosConfig, options, accountAddress } = args;
    const gracePeriod = await getANSGracePeriod({ aptosConfig });
    const expirationDate = await getANSExpirationDate({ aptosConfig });
    const data = await queryIndexer({
 ...
 * @group Implementation
 */
export async function getAccountSubdomains(args) {
    const { aptosConfig, options, accountAddress } = args;
    const gracePeriod = await getANSGracePeriod({ aptosConfig });
    const data = await queryIndexer({
        aptosConfig,
 ...
 * @group Implementation
 */
export async function getDomainSubdomains(args) {
 ...
 * @group Implementation
 */
async function getANSExpirationDate(args) {
 ...
 * @group Implementation
 */
export async function getANSGracePeriod(args) {
    if (GRACE_PERIOD_IN_SECONDS) {
        return GRACE_PERIOD_IN_SECONDS;
    }
    const { aptosConfig } = args;
    const routerAddress = getRouterAddress(aptosConfig);
    const [gracePeriodInSeconds] = await view({
        aptosConfig,
        payload: {
 ...
 * @group Implementation
 */
function sanitizeANSName({ name, aptosConfig, gracePeriod, }) {
    ...
    const expiration = isSubdomain && expirationPolicy === SubdomainExpirationPolicy.FollowsDomain
