...
import { TransactionResponseType, } from "../types/index.js";
 ...
 * @group Implementation
 */
export async function getGasPriceEstimation(args) {
    ...
    return memoizeAsync(async () => {
        ...
        return data;
    }, `gas-price-${aptosConfig.network}`, 1000 * 60 * 5)();
}
/**
...
export async function waitForTransaction(args) {
    ...
    function isUnsettled(txn) {
        if (txn === undefined)
            return true;
        if (txn.type === TransactionResponseType.Pending)
            return true;
        return txn.success === undefined;
    }
    /**
    ...
    if (lastTxn.success === undefined) {
        throw new WaitForTransactionError(`Transaction ${transactionHash} did not finish indexing within ${timeoutSecs} seconds`, lastTxn);
    }
    if (!checkSuccess) {
        return lastTxn;
    }
...
export async function getBlockByVersion(args) {
    ...
    const { data: block } = await getAptosFullNode({
        aptosConfig,
...
async function fillBlockTransactions(args) {
    ...
    if (options?.withTransactions) {
        ...
        const curVersion = lastTxn && "version" in lastTxn ? lastTxn.version : undefined;
        let latestVersion;
        ...
        for (let i = latestVersion + 1n; i < lastVersion; i += BigInt(100)) {
            fetchFutures.push(getTransactions({
                aptosConfig,
                options: {
                    offset: i,
