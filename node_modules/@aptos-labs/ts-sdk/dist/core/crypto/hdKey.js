 ...
 * @category Serialization
 */
export function isValidBIP44Path(path) {
 ...
 * @category Serialization
 */
export function isValidHardenedPath(path) {
 ...
 * @category Serialization
 */
const toBytes = (input) => typeof input === "string" ? TEXT_ENCODER.encode(input) : input;
export const deriveKey = (hashSeed, data) => {
...
const removeApostrophes = (val) => val.replace(/'/g, "");
/**
 ...
 * @category Serialization
 */
export const splitPath = (path) => path.split("/").slice(1).map(removeApostrophes);
/**
