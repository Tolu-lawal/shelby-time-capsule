 ...
 */
export function hashStrToField(str, maxSizeBytes) {
 ...
 * @category Serialization
 */
export function padAndPackBytesWithLen(bytes, maxSizeBytes) {
    if (bytes.length > maxSizeBytes) {
        throw new Error(`Input bytes of length ${bytes.length} is longer than ${maxSizeBytes}`);
    }
...
export function poseidonHash(inputs) {
    if (inputs.length === 0 || inputs.length > numInputsToPoseidonFunc.length) {
        throw new Error(`poseidonHash requires between 1 and ${numInputsToPoseidonFunc.length} inputs, got ${inputs.length}`);
    }
