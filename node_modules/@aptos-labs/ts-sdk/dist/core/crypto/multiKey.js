import { AnyPublicKeyVariant, SigningScheme as AuthenticationKeyScheme } from "../../types/index.js";
...
export class AbstractMultiKey extends AccountPublicKey {
    publicKeys;
    constructor(args) {
        super();
...
export class MultiKey extends AbstractMultiKey {
    ...
    constructor(args) {
        ...
        this.publicKeys = publicKeys.map((publicKey) => publicKey instanceof AnyPublicKey ? publicKey : new AnyPublicKey(publicKey));
        if (signaturesRequired > MAX_NUM_KEYLESS_PUBLIC_FOR_MULTI_KEY) {
            const keylessCount = this.publicKeys.filter((pk) => pk.variant === AnyPublicKeyVariant.Keyless || pk.variant === AnyPublicKeyVariant.FederatedKeyless).length;
            if (keylessCount > MAX_NUM_KEYLESS_PUBLIC_FOR_MULTI_KEY) {
                throw new Error(`Construction of MultiKey with more than ${MAX_NUM_KEYLESS_PUBLIC_FOR_MULTI_KEY} keyless public keys is not allowed when signaturesRequired 
...
export class MultiKeySignature extends Signature {
    ...
    constructor(args) {
        ...
        this.signatures = signatures.map((signature) => signature instanceof AnySignature ? signature : new AnySignature(signature));
