...
import { Serializable, serializeEntryFunctionBytesCompat } from "../bcs/serializer.js";
import { ParsingError } from "./common.js";
 ...
 */
export var AddressInvalidReason;
(function (AddressInvalidReason) {
    ...
    AddressInvalidReason["INVALID_PADDING_STRICTNESS"] = "INVALID_PADDING_STRICTNESS";
...
export class AccountAddress extends Serializable {
     ...
     * @category Serialization
     */
    static LONG_STRING_LENGTH = 64;
    static ZERO = AccountAddress.from("0x0");
    static ONE = AccountAddress.from("0x1");
    static TWO = AccountAddress.from("0x2");
    static THREE = AccountAddress.from("0x3");
    static FOUR = AccountAddress.from("0x4");
    static A = AccountAddress.from("0xA");
    /**
    ...
    constructor(input) {
        ...
        if (input.length !== AccountAddress.LENGTH) {
            throw new ParsingError("AccountAddress data should be exactly 32 bytes long", AddressInvalidReason.INCORRECT_NUMBER_OF_BYTES);
        }
        this.data = input;
    }
    /**
    ...
    static fromStringStrict(input) {
        ...
        if (input.length !== AccountAddress.LONG_STRING_LENGTH + 2) {
            if (!address.isSpecial()) {
                throw new ParsingError(`The given hex string ${input} is not a special address, it must be represented as 0x + 64 chars.`, AddressInvalidReason.LONG_FORM_REQUIRED_UNLESS_SPECIAL);
            }
            else if (input.length !== 3) {
                // 0x + one hex char is the only valid SHORT form for special addresses.
                throw new ParsingError(`The given hex string ${input} is a special address not in LONG form, it must be 0x0 to 0xf without padding zeroes.`, AddressInvalidReason.INVALID_PADDING_ZEROES);
            }
        }
        return address;
    }
    ...
    static fromString(input, { maxMissingChars = 4 } = {}) {
        ...
        if (parsedInput.length === 0) {
            throw new ParsingError("Hex string is too short, must be 1 to 64 chars long, excluding the leading 0x.", AddressInvalidReason.TOO_SHORT);
        }
        // Ensure the address string is not longer than 64 characters.
        if (parsedInput.length > 64) {
            throw new ParsingError("Hex string is too long, must be 1 to 64 chars long, excluding the leading 0x.", AddressInvalidReason.TOO_LONG);
        }
        // Ensure that the maxMissingChars is between or equal to 0 and 63.
        if (maxMissingChars > 63 || maxMissingChars < 0) {
            throw new ParsingError(`maxMissingChars must be between or equal to 0 and 63. Received ${maxMissingChars}`, AddressInvalidReason.INVALID_PADDING_STRICTNESS);
        }
        let addressBytes;
        try {
        ...
        if (parsedInput.length < 64 - maxMissingChars) {
            if (!address.isSpecial()) {
                throw new ParsingError(`Hex string is too short, must be ${64 - maxMissingChars} to 64 chars long, excluding the leading 0x. You may need to fix 
the addresss by padding it with 0s before passing it to \`fromString\` (e.g. <addressString>.padStart(64, '0')). 
Received ${input}`, AddressInvalidReason.TOO_SHORT);
            }
        }
        return address;
    }
