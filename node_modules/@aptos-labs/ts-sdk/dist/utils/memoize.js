...
function validateCacheKey(key) {
    ...
    if (key.length < MIN_KEY_LENGTH) {
        throw new Error(`Cache key "${key}" is too short. Keys should be at least ${MIN_KEY_LENGTH} characters and include a namespace prefix.`);
    }
 ...
 * @category Utils
 */
export function memoizeAsync(func, key, ttlMs) {
 ...
 * @category Utils
 */
export function memoize(func, key, ttlMs) {
    // Validate key format on first call to catch misuse early
    validateCacheKey(key);
    return (...args) => {
        const now = Date.now();
 ...
 * @category Utils
 */
export function getMemoizeCacheSize() {
    return cache.size;
}
