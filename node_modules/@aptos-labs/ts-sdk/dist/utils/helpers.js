 ...
 */
export function u64ToNumberSafe(value, fieldName) {
    if (value < 0n) {
        throw new RangeError(`${fieldName} is negative (${value}); expected an unsigned u64`);
    }
    if (value > MAX_SAFE_U64) {
        throw new RangeError(`${fieldName} (${value}) exceeds Number.MAX_SAFE_INTEGER (${MAX_SAFE_U64}); refusing to silently lose precision`);
    }
    return Number(value);
}
/**
 ...
 * @category Utils
 */
export function warnIfDevelopment(message) {
 ...
 * @category Utils
 */
export const convertAmountFromHumanReadableToOnChain = (value, decimal) => value * 10 ** decimal;
/**
 * Amount is represented in the smallest unit format on chain, this function converts
 ...
 * @category Utils
 */
export const convertAmountFromOnChainToHumanReadable = (value, decimal) => value / 10 ** decimal;
/**
 ...
 * @category Utils
 */
export const isEncodedStruct = (structObj) => typeof structObj === "object" &&
 ...
 * @category Utils
 */
export function isValidFunctionInfo(functionInfo) {
 ...
 * @category Utils
 */
export function truncateAddress(address, start = 6, end = 5) {
 ...
 */
export function pairedFaMetadataAddress(coinType) {
